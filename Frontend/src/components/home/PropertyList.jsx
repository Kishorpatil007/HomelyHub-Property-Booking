...
const PropertyList = () => {
  ...
  const lastPage = totalProperties > 0 ? Math.ceil(totalProperties / 12) : 1;

  const propertyListRef = useRef(null);

  useEffect(() => {
    const fetchProperties = async () => {
      dispatch(propertyActions.updateSearchParams({ page: currentPage }));
  ...
  return (
    <>
      ...
      <div className="pagination">
        ...
        <button
          className="next_btn"
          onClick={() => setCurrentPage((prev) => Math.min(prev + 1, lastPage || prev))}
