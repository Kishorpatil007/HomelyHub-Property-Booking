  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        if (!value.maximumGuest || Number(value.maximumGuest) <= 0) {
          toast.error("Please give maximum no of guests that can occupy.");
          return;
        }

        if (!Array.isArray(value.images) || value.images.length < 6) {
          toast.error("Please upload at least 6 photos before publishing.");
          return;
        }

        const normalizedAmenities = (value.amenities || []).map((amenity) => {
          const name = typeof amenity === 'string' ? amenity : amenity.name;
          return {
            name: String(name).trim().toLowerCase(),
            icon: typeof amenity === 'string' ? 'check' : (amenity.icon || 'check')
          };
        });
        ...
        await dispatch(
          createAccomodation({
            ...
            propertyType: String(value.propertyType || '').trim().toLowerCase(),
