...
const createProperty = async (req, res) => {
  try {
    ...
    const normalizedPropertyName = propertyName || req.body.name;
    const normalizedPropertyType = String(propertyType || "").trim().toLowerCase();
    const normalizedMaximumGuest = Number(maximumGuest ?? maximumGuests ?? 0);
    ...
    const property = await Property.create({
      ...
      checkInTime,
      checkOutTime,
      maximumGuest: normalizedMaximumGuest,
      price,
